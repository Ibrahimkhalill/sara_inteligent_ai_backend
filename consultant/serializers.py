...
from .models import Member, ConsultantFarm, ConsultantRequest
...
class ConsultantRequestSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = ['id', 'farm', 'farm_email', 'farm_name', 'consultant', 'consultant_email', 'consultant_name', 'status', 'created_at', 'updated_at']
    ...
    def validate(self, data):
        ...
        if farm == consultant:
            errors['errors'] = ['Farm and consultant cannot be the same user']

        if ConsultantFarm.objects.filter(farm=farm, consultant=consultant, is_active=True).exists():
            errors['errors'] = ['This consultant is already associated with the farm']

        if ConsultantRequest.objects.filter(farm=farm, consultant=consultant, status='pending').exists():
            errors['errors'] = ['A pending request already exists for this consultant and farm']
